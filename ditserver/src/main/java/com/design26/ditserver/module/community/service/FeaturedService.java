...
import com.design26.ditserver.module.community.repository.FeaturedPublicationRepository;
...
/**
 * FeaturedService - 精选（featured）功能的业务层。
 *
 * 提供对 featured_publications 表的聚合查询，用于供控制器返回前端需要的结构。
 */
@Service
public class FeaturedService {
    private final FeaturedPublicationRepository featuredRepository;
    ...
    public FeaturedService(
        FeaturedPublicationRepository featuredRepository,
    ...
    public Map<String, Object> listFeatured() {
        List<FeaturedPublicationEntity> pubs = featuredRepository.findByIsActiveTrueOrderByUpdatedAtDesc();
        List<String> archiveIds = pubs.stream().map(item -> item.getArchive().getId()).toList();

        Map<String, List<com.design26.ditserver.module.workspace.entity.ArchiveMessageEntity>> messagesByArchive = archiveIds.isEmpty()
            ? Map.of()
        ...
        Map<String, List<com.design26.ditserver.module.workspace.entity.ArchiveTaskEntity>> tasksByArchive = archiveIds.isEmpty()
            ? Map.of()
